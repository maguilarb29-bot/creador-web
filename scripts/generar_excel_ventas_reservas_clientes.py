...
OUT_PATH = BASE / "docs" / "Ventas_Reservas_Clientes_2026-04-19.xlsx"
...
CLIENTE_COLORS = {
    ...
    "Pablo Arias":      {"dark": "2E7D32", "light": "E8F5E9"},
    "Marielos Alpizar": {"dark": "4A148C", "light": "F3E5F5"},
    "Ana Isabel Beche": {"dark": "00695C", "light": "E0F2F1"},
...
HEREDEROS = {
    ...
    "Margherita Pignatelli", "Maria Cristina Pignatelli",
}
...
with open(BASE / "Api_PG/data/clientes.json", encoding="utf-8-sig") as f:
    clientes_db = json.load(f)

# Lookup telefono por nombre completo
def get_tel(nombre):
    for c in clientes_db:
        nombre_completo = (c["nombre"] + " " + c["apellido"]).strip()
        if nombre_completo == nombre or c["nombre"] == nombre:
            return c.get("telefono", "")
    return ""
...
COLS_V = 9
for col, w in zip("ABCDEFGHI", [5, 10, 44, 20, 16, 14, 16, 18, 14]):
...
for col, h in enumerate(
        ["N°", "CODIGO", "DESCRIPCION", "COMPRADOR", "TELEFONO",
         "CATEGORIA", "PRECIO USD", "PRECIO CRC", "FECHA VENTA"], 1):
...
for cli in sorted(ventas_por_cli.keys()):
    ...
    banda(ws, row_n, COLS_V,
          f"  {cli.upper()}{tel_txt}  —  {len(items)} articulo{'s' if len(items)>1 else ''}",
          colors["dark"])
    row_n += 1
    ...
    for idx, a in enumerate(items):
        ...
        cell(ws, row_n, 5, tel,                                    bg=bg, fg="37474F", size=9, center=True)
        cell(ws, row_n, 6, a.get("categoria", ""),                 bg=bg, fg="666666", size=8, center=True)
        cell(ws, row_n, 7, f"$ {usd:,.2f}" if usd else "",        bg=bg, center=True, bold=True, fg="1F4E79")
        cell(ws, row_n, 8, f"CRC {crc:,.2f}" if crc else "",      bg=bg, center=True, bold=True, fg="27AE60", size=9)
        cell(ws, row_n, 9, a.get("fechaVenta", ""),                bg=bg, center=True, fg="555555", size=9)
        ws.row_dimensions[row_n].height = 18
        row_n += 1

total_row(ws, row_n, 6, f"  TOTAL - {num} articulos vendidos",
          [(7, f"$ {tot_usd:,.2f}"), (8, f"CRC {tot_crc:,.2f}"), (9, "")])
...
COLS_R = 7
for col, w in zip("ABCDEFG", [5, 10, 44, 20, 16, 14, 22]):
...
for col, h in enumerate(
        ["N°", "CODIGO", "DESCRIPCION", "CLIENTE", "TELEFONO",
         "CATEGORIA", "PRECIO SUGERIDO (USD)"], 1):
...
for cli in sorted(reservas_por_cli.keys()):
    ...
    for idx, a in enumerate(items):
        ...
        cell(ws2, row_n, 3, a.get("nombreES", "").capitalize()[:55], bg=bg, size=9)
        cell(ws2, row_n, 4, cli,                                     bg=bg, bold=True, fg=colors["dark"], size=9)
        cell(ws2, row_n, 5, tel,                                     bg=bg, fg="37474F", size=9, center=True)
        cell(ws2, row_n, 6, a.get("categoria", ""),                  bg=bg, fg="666666", size=8, center=True)
        cell(ws2, row_n, 7,
